...
RSpec.describe RandomNameGenerator do
  ...
  it "has a version number" do
    expect(RandomNameGenerator::VERSION).not_to be_nil
  end

  context "with languages" do
    it "when it has an Elvish language" do
      expect(RandomNameGenerator::ELVEN).to be_a(File)
      expect(RandomNameGenerator::ELVEN.path).to include("languages/elven.txt")
    end

    it "when it has an Elvish Russian language" do
      expect(RandomNameGenerator::ELVEN_RU).to be_a(File)
      expect(RandomNameGenerator::ELVEN_RU.path).to include("languages/elven-ru.txt")
    end

    it "when it has an Fantasy language" do
      expect(RandomNameGenerator::FANTASY).to be_a(File)
      expect(RandomNameGenerator::FANTASY.path).to include("languages/fantasy.txt")
    end

    it "when it has an Fantasy Russian language" do
      expect(RandomNameGenerator::FANTASY_RU).to be_a(File)
      expect(RandomNameGenerator::FANTASY_RU.path).to include("languages/fantasy-ru.txt")
    end

    it "when it has an Goblin language" do
      expect(RandomNameGenerator::GOBLIN).to be_a(File)
      expect(RandomNameGenerator::GOBLIN.path).to include("languages/goblin.txt")
    end

    it "when it has an Goblin Russian language" do
      expect(RandomNameGenerator::GOBLIN_RU).to be_a(File)
      expect(RandomNameGenerator::GOBLIN_RU.path).to include("languages/goblin-ru.txt")
    end

    it "when it has an Roman language" do
      expect(RandomNameGenerator::ROMAN).to be_a(File)
      expect(RandomNameGenerator::ROMAN.path).to include("languages/roman.txt")
    end

    it "when it has an Roman Russian language" do
  ...
  describe RandomNameGenerator::Generator do
    ...
    context "with scratch" do
      it "when it does stuff" do
        expect(@tiny.compose).not_to be_nil
